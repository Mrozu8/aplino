...
//    ===================== zatwierdzanie formularza ================
    ...
    public function edit_end(Request $request, $id, $form_id)
    {
        ...
        return redirect()->route('company-form', $id)->with('status-error', 'Błąd potwierdzenia');
...
//    ======================================= Show gotowego do wypełnienia CV =====================
    ...
    public function show_cv($company_name, $form_id){
        ...
        if($form->active == 1)
        {
            ...
            $business_card = BusinessCard::where('user_id', $form->user->id)->first();
...
//    ======================================== podgląd gotowego cv ====================
    ...
    public function show_pre_cv($company_name, $form_id){
        ...
        return view('form.pre-cv', compact('form', 'customs', 'business_card'));
    }
...
//        dd($cv);
        ...
        if($cv == true)
        {
            return back()->with('status-error', 'Aplikowałeś już na to stanowisko.');
        }
        ...
        foreach($req as $key => $value){
            if($key !== '_token' && $key !== '_method' && $key !== 'basic' && $key !== 'file' && $key != 'p-data' && $key != 'rule'){
...
//    ================================== aktywacja formularza ===================
    ...
    public function active_form($id, $form_id)
    {
        ...
        $form = Form::select('active_date', 'id')->where([
            ['id', $form_id],
            ['user_id', Auth::id()]
        ])->first();
        ...
        else
        {
            if($form->active_date != null)
            {
                ...
                $form = Form::where([
                    ['id', $form_id],
                    ['user_id', Auth::id()]
                ])->update(['active_date' => $date, 'active' => 1]);
...
//                $form_id = $form['id'];
                ...
                $status = "Ważności Twojego formularza została wydłużona o 1 miesiąc";

//                dd($user_id);

                Mail::to(Auth::user()->email)->send(new Approv($form_id, $user_id, $company_name, $status));
            ...
            else
            {
                ...
                $user_id = $user->id;
                $company_name = $user->company_name;
                $status = "Twój formularz został pozytywnie zatwierdzony";
...
//        $user_package = User::select('package')->where('id', Auth::id())->first();
...
//            ])->update(['active' => 0]);
//
//            return back()->with('status-success', 'Formularz został dezaktywowany'); // zmiana statusu formulrza na - nieaktywny
//        }
//        elseif($request->status == 1)
//        {
...
//            {
//
//                return back()->with('status-error', 'Aby aktywować formularz, zmień status konta'); // Brak konta premium - wszystkie formularze - nieaktywne
//            }
...
//                    ])->update(['active' => 1]);
//
//                    return back()->with('status-success', 'Formularz został aktywowany'); // zmiana statusu na - active
//                }
//                else
//                {
//                    return back()->with('status-error', 'Za dużo aktywnych formularzy, zmień status konta, lub dezaktywuj inny formularz'); // zbyt dużo aktywnych formularzy
//                }
//            }
...
//
//
//        return back()->with('status-error', 'Coś poszło nie tak'); // błędne dane w formularzu
